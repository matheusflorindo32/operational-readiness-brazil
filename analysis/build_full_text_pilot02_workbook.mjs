...
const source = JSON.parse(await fs.readFile(path.join(os.tmpdir(), "orb-pilot-02-workbook-source.json"), "utf8"));
const outputPath = "outputs/full-text/2026-09-17/Operational_Readiness_Full_Text_Pilot_02.xlsx";
const artifactPath = "reporting/full-text/2026-09-17/pilot-02/workbook-artifact.json";
...
summary.getRange("A2").values = [["Full-Text Evidence Qualification — Piloto 02"]];
...
summary.getRange("A5:B18").values = [
  ...
  ["Versões de registro", 7], ["AAM explícitos", 3], ["Estudos qualitativos", 4], ["Mixed methods", 2],
  ["Quase experimentais", 4], ["Appraisals IA provisórios", 10], ["Red Team PASS", 10],
  ["Confirmações humanas", null], ["Claim-Ready", null], ["PMC ativos processados", "20/383"],
];
...
summary.getRange("D6:H7").merge(); summary.getRange("D6").values = [["GO_PILOT_03"]];
summary.getRange("D6:H7").format.fill = paleGreen; summary.getRange("D6:H7").format.font = { name: font, bold: true, size: 14, color: "#1D5C2E" };
summary.getRange("D9:H12").merge(); summary.getRange("D9").values = [["Processamento técnico concluído. Todos os julgamentos de appraisal são provisórios e assistidos por IA. Os 20 registros dos Pilotos 01 e 02 permanecem em revisão humana."]];
summary.getRange("D9:H12").format.fill = paleAmber;
summary.getRange("D14:H16").merge(); summary.getRange("D14").values = [["PASS científico: PROIBIDO | Claim-Ready: 0/10 | Confirmação humana: 0/20"]];
...
summary.getRange("A20:C24").values = [
  ...
  ["JBI Quasi-Experimental", "https://jbi.global/sites/default/files/2026-05/2_JBI%20checklist%20for%20quasi-experimental%20studies.docx", "Quatro estudos quase experimentais"],
  ["MMAT 2018", "https://mixedmethodsappraisaltoolpublic.pbworks.com/w/file/fetch/127916259/MMAT_2018_criteria%20manual_2018%20-08-01_ENG.pdf", "Dois estudos mistos; componentes completos"],
];
summary.getRange("A20:C20").format.fill = navy; summary.getRange("A20:C20").format.font = { name: font, bold: true, color: "#FFFFFF" };
summary.getRange("A20:C24").format.wrapText = true; summary.getRange("B:B").format.columnWidth = 55; summary.getRange("C:C").format.columnWidth = 38;
summary.tabColor = blue;

const selectionFields = ["pilot_order","evidence_id","pmid","priority_tier","priority_score","access_status","queue_position_after_pilot01_exclusion","selection_rule"];
const selection = wb.worksheets.add("Seleção");
selection.getRange("A1").write(matrix(source.selection, selectionFields, ["Ordem","Evidence ID","PMID","Prioridade","Score","Acesso","Posição após excluir Piloto 01","Regra determinística"]));
...
const versionFields = ["evidence_id","pmid","pmcid","source_version","publisher_url","publisher_access","vor_comparison","claim_location_status","human_confirmed"];
const versions = wb.worksheets.add("Versões");
versions.getRange("A1").write(matrix(source.versions, versionFields, ["Evidence ID","PMID","PMCID","Versão fonte","URL publisher","Acesso publisher","Comparação VOR","Localização de claim","Confirmação humana"]));
styleTable(versions, versions.getUsedRange(), 3); versions.getRange("D:H").format.columnWidth = 42; versions.getRange("I2:I11").format.fill = paleAmber; versions.getRange("A1:I11").format.wrapText = true;
...
const humanFields = ["pilot","evidence_id","pmid","title","ai_decision_provisional","design","evidence_class","instrument","main_findings","limitations","supported_claims","unsupported_claims","pending_items","human_reviewer","human_review_date","human_decision","human_justification","human_confirmation","claim_ready"];
const human = wb.worksheets.add("Revisão humana");
human.getRange("A1").write(matrix(source.humanQueue, humanFields, ["Piloto","Evidence ID","PMID","Título","Decisão IA provisória","Desenho","Classe","Instrumento","Principais achados","Limitações","Supported claims","Unsupported claims","Pendências","Revisor humano","Data humana","Decisão humana","Justificativa humana","Confirmação humana","Claim-Ready"]));
styleTable(human, human.getUsedRange(), 3); human.getRange("D:M").format.columnWidth = 40; human.getRange("N:R").format.columnWidth = 25; human.getRange("N2:R21").format.fill = paleAmber; human.getRange("A1:S21").format.wrapText = true;
human.getRange("P2:P21").dataValidation = { rule: { type: "list", values: ["Confirmar retenção", "Confirmar exclusão", "Solicitar esclarecimento", "Solicitar nova avaliação"] } };
human.getRange("R2:R21").dataValidation = { rule: { type: "list", values: ["YES", "NO"] } };

summary.getRange("B16").formulas = [["=COUNTIF('Revisão humana'!$R$2:$R$21,\"YES\")"]];
...
const previewDir = path.join(os.tmpdir(), "orb-pilot-02-previews");
...
const artifact = {
  ...
  summary: { records: 10, full_text: 10, identity: 10, appraisals: 10, red_team: 10, human_confirmations: 0, claim_ready: 0, decision: "GO_PILOT_03" },
};
